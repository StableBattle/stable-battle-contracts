...
contract RegenLibs is Script {
    ...
    function formAddressLine(address newAddress, string memory libName)
        private
        pure
        returns (string memory addressLine)
    {
        ...
        addressLine = beginLine.concat(libNameSlice).toSlice().concat(middleLine).toSlice().concat(newAddressSlice)
            .toSlice().concat(endLine);
    ...
    function formSolFileIntro(string memory libName) private pure returns (string memory solFileIntro) {
        strings.slice memory beginIntro =
            "// SPDX-License-Identifier: None\n\npragma solidity ^0.8.0;\n\nlibrary ".toSlice();
        strings.slice memory libNameSlice = libName.toSlice();
