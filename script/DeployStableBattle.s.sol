...
contract DeployStableBattle is DiamondHelper, IDeployErrors {
    ...
    function deployBEER(address owner, bytes32 salt) public returns (IBEER) {
        ...
        IUpgradeableProxyOwnable(payable(address(BEERProxyContract))).setImplementation(
            address(BEERImplementationContract)
        );
    ...
    function deploySBV(address owner, bytes32 salt) public returns (ISBV) {
        ...
        IUpgradeableProxyOwnable(payable(address(SBVProxyContract))).setImplementation(
            address(SBVImplementationContract)
        );
    ...
    function deployStableBattle(address owner, bytes32 salt) public returns (IStableBattle, IBEER, ISBV) {
        ...
        if (verbose) {
            for (uint256 i = 0; i < cut.length; i++) {
                ...
                console2.logUint(uint256(cut[i].action));
    ...
    function deployFacets() internal returns (IDiamondCut.FacetCut[] memory cut) {
        ...
        cut[6] = (
            IDiamondCut.FacetCut({
                ...
                removeElement(
                    bytes4(keccak256(bytes("supportsInterface(bytes4)"))), generateSelectors(type(ItemsFacet).name)
                    )
            })
        );
